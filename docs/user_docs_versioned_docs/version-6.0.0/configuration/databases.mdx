...
# Optional scope validation for each client: "ignore" (default), "subset",
# or "exact". Add "scope_matching_policy" to a client block.
...
##### BigQuery OAuth impersonation

When BigQuery is configured for per-user OAuth impersonation, Superset requests the
`https://www.googleapis.com/auth/bigquery.readonly` scope by default. This keeps tokens
issued by Superset read-only even when the impersonated user has write permissions in
other contexts. Returned scopes are matched exactly by default for impersonated BigQuery
connections; an explicit per-client `scope_matching_policy` can override this behavior.

To allow impersonated users to perform BigQuery write operations through Superset,
explicitly opt in from `superset_config.py`:

```python
BIGQUERY_IMPERSONATION_ALLOW_WRITE = True
```

Changing this setting invalidates incompatible stored tokens. Users must authorize
again so Superset can obtain a token with the configured scope. BigQuery file uploads
also require this write opt-in, `allow_file_upload` on the database, the required
BigQuery IAM permissions, and the [pandas_gbq](https://github.com/pydata/pandas-gbq)
library.

Metadata caching is disabled for impersonated databases because catalog, schema, and
table visibility can depend on the authorized Google identity.
